...
void main() {
  ...
  // Amplify gradient for dramatic interfacial instabilities
  float gradMag = length(gradTh);
  vec2 Ft = normalize(gradTh + 1e-6) * u_k_th * (1.0 + gradMag * 5.0);

  // Scale, gate with higher clamp for more dramatic effects
  float clampMag = 0.15; // allow stronger forces
